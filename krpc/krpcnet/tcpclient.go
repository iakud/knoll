...
func (c *tcpClient) Receive(tcpconn *knet.TCPConn, data []byte) {
	...
	if m.Header().FlagReply() && conn.rt != nil {
		if err := conn.rt.handleReply(m); err != nil {
			slog.Info("krpcnet: tcpclient handle reply", "error", err)
		}
		return
	}

	if err := c.handleMessage(conn, m); err != nil {
		slog.Info("krpcnet: tcpclient handle msg", "msgId", m.Header().MsgId(), "error", err)
	}
}
...
func (c *tcpClient) handleMessage(conn *tcpConn, m Msg) error {
	...
	return c.handler.Receive(conn, m)
}
